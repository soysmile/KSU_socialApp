...
class SettingsViewController: UIViewController{
    ...
    func userInteraction(){
        ...
        let tapDataChangeSchedule = UITapGestureRecognizer(target: self, action: #selector(self.touchTappedToSchedule(_:)))
        
        dataDisplayView.addGestureRecognizer(tapDataChange)
        schedule.addGestureRecognizer(tapDataChangeSchedule)
    }
    
    @objc func touchTapped(_ sender: UITapGestureRecognizer) {
        let nextVC = self.storyboard?.instantiateViewController(withIdentifier: "changeDataVC") as! UIViewController
        self.navigationController?.pushViewController(nextVC, animated: true)
    }

    @objc func touchTappedToSchedule(_ sender: UITapGestureRecognizer) {
        let toSchedule = self.storyboard?.instantiateViewController(withIdentifier: "toSchedule") as! UIViewController
        self.navigationController?.pushViewController(toSchedule, animated: true)
    }
